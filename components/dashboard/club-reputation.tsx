...
export default function ClubReputation({leagueName}: { leagueName: string | null }) {
  ...
  if (error) return <Alert variant="destructive" className="my-4"><AlertCircle
      className="h-4 w-4"/><AlertTitle>Error</AlertTitle><AlertDescription>{error}</AlertDescription></Alert>;
  if (!leagueName || data.length === 0) {
    return (
        <Card><CardHeader><CardTitle>Club Reputation</CardTitle></CardHeader><CardContent><p
            className="text-center text-muted-foreground py-10">No review data available for this league yet.</p>
        </CardContent></Card>
    );
  }
        ...
        <div className="space-y-6">
            ...
            <CardHeader>
              <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-4">
                <div>
                  ...
                  <CardDescription>Aggregated player reviews on key off-pitch factors. Based
                    on {data.reduce((sum, club) => sum + club.reviewCount, 0)} total reviews.</CardDescription>
                </div>
                <Select
              ...
              <div className="h-[450px]">
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={sortedChartData} layout="vertical" margin={{top: 5, right: 30, left: 80, bottom: 5}}>
                    ...
                    />
                    <Bar dataKey={selectedMetric} name={metricDisplayNames[selectedMetric]} fill="#31348D"
                         barSize={20}/>
                  </BarChart>
                </ResponsiveContainer>
              </div>
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>Based on the weighted average of all player reviews. Click a club to see
                comments.</CardDescription>
            </CardHeader>
            <CardContent>
                ...
                {selectedClub?.reviews.map(review => (
                    <div key={review.id} className="border-b pb-4 last:border-b-0">
                      ...
                      <div className="mt-4 space-y-1">
                        <StarRating rating={review.category_ratings?.["Salary Punctuality"] || 0}
                                    label="Salary Punctuality"/>
                        <StarRating rating={review.category_ratings?.["Fair Salary"] || 0} label="Fair Salary"/>
                        <StarRating rating={review.category_ratings?.["Training Conditions"] || 0}
