...
BEGIN
  ...
  WITH player_matches AS (
    ...
    CROSS JOIN (
      SELECT p.id, p.name, p.main_position, p.age, p.height, p.foot, ar.agent_id
      FROM agent_rosters ar
      JOIN players_transfermarkt p ON ar.player_id = p.id
      WHERE ar.agent_id = p_agent_id
    ) p
    LEFT JOIN agent_player_overrides apo ON apo.agent_id = p.agent_id AND apo.player_id = p.id
  ...
  SELECT
    ...
    pm.matched_player_id::INTEGER,
    pm.matched_player_name::TEXT,
    jsonb_build_object(
      'position_match_type', CASE WHEN pm.position_match THEN 'exact' ELSE 'none' END,
      'age_match_type', CASE WHEN pm.age_match THEN 'exact' ELSE 'none' END,
      'height_match_type', CASE WHEN pm.height_match THEN 'exact' ELSE 'none' END,
      'foot_match_type', CASE WHEN pm.foot_match THEN 'exact' ELSE 'none' END,
